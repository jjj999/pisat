...
コンポーネントとは，直感的には**pisat システムにおける部品**と理解されます．
つまり，コンポーネントとはミッション遂行のための道具と変わりありません．
道具や道具と言われるとハードウェアを想像するかもしれませんが，
コンポーネントの大半はハードウェアをソフトウェア的に実装したものです．
一方で，ハードウェア的な実体のないようなコンポーネントもいくつか存在します．
どちらも pisat システムにおける道具という位置づけには代わりはありません．

コンポーネントとは厳密には，pisat.base.Component クラスのサブクラスのオブジェクトのことを指します．
このことさえ遵守すれば，ハードウェア的な実体のないコンポーネントを作ることが出来ます．
逆に Component クラスを継承したクラスを実装しなければ，ハードウェア的な実体が存在したとしても，
そのクラスのオブジェクトはコンポーネントと呼べません．
背景がどうであれ，Component クラスを継承すればコンポーネントを作成することは可能だということは
覚えておいてください．

pisat の様々なサブクラスでは Component クラスを継承した抽象クラスが用意されています．
ここでは抽象クラスとは，不完全な実装を持つメソッドが存在するクラスのことを指しています．
例えば，センサーを実装するための抽象クラスとして pisat.sensor.SensorBase などがあります．
このような抽象クラスは実装が不完全なので，そのまま利用することは出来ません．
ユーザーが独自にコンポーネントを作成したい場合は，このような抽象クラスを継承し，
不完全な変数やメソッドを実装する必要があります．
...
ハンドラは様々な GPIO などのハードウェアインターフェースを操作する
...
これらのハンドラの抽象クラスをミドルウェアを用いて実装することで，
...
InputHandlerBase，OutputHandlerBase などはその具象クラス
(抽象クラスを実装したクラス) をそのまま利用することもあるでしょう．
...
[初回のドキュメント](pkg_overview.md)の「サブパッケージ間の依存関係」を参照してください．
...
pisat.sensor.SensorBase は全てのセンサークラスが継承すべき抽象クラスです．
...
readf メソッドと read メソッドは引数として可変長のデータ名 (str) を受け付けます．
...
上述したようにラップしてパフォーマンスが低下するのなら，
なぜラップするのかと疑問になるかもしれません．
在野のクラスをラップしセンサークラスとする最大の理由は，
pisat システムが許容するインターフェイスを備えるためにあります．

pisat システムでは，Node.judge コールバックに DataLogger を用いて取得した
センサー値などを引数として渡すという話を[前々回のドキュメント](system.md)
で説明しました．
このとき DataLogger は SensorBase のインターフェイスを利用して
センサー値の取得を行います．
つまり，上記の例のように SensorBase とは異なるインターフェイス 
(上の例では read_temp) ではセンサーを統合的に扱うことが出来ないわけです．

このように，pisat システムを利用する場合には，センサークラスは必ず SensorBase 
を継承し実装する必要があります．
逆に，pisat システムを利用しない場合はラップする必要はありません．
